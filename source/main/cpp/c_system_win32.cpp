...
namespace ncore
{
    namespace nsystem
    {
        ...
        static ctxt_t* sCtxt = NULL;

        static void        InitializeExecutableInfo(ctxt_t* c);
        static void        GenerateExceptionReport(ctxt_t* c, PEXCEPTION_POINTERS pExceptionInfo);
        static LONG WINAPI UnhandledExceptionFilter(ctxt_t* c, PEXCEPTION_POINTERS pExceptionInfo);
        ...
        //---------------------------------------------------------------------------------------------------------------------
        void InitializeExecutableInfo(ctxt_t* c)
        {
            HMODULE cb[64];
...
#    endif
        ...
        //---------------------------------------------------------------------------------------------------------------------
        u64 getCodeSegmentSize(ctxt_t* c) { return c->m_codeSegmentSize; }

        //---------------------------------------------------------------------------------------------------------------------

        u64 getBssSegmentSize(ctxt_t* c) { return c->m_bssSegmentSize; }

        //---------------------------------------------------------------------------------------------------------------------

        u64 getDataSegmentSize(ctxt_t* c) { return c->m_dataSegmentSize; }

        //---------------------------------------------------------------------------------------------------------------------

        u64 getMainThreadStackSize(ctxt_t* c) { return c->m_mainThreadStackSize; }

        //---------------------------------------------------------------------------------------------------------------------

        const char* getExePath(ctxt_t* c) { return c->m_szExePath; }

        const char* getNickname(ctxt_t* c) { return c->m_szNickName; }

        void setGameTitle(ctxt_t* c, const char* title) { ascii::copy(c->m_szAppTitle, c->m_szAppTitle + sizeof(c->m_szAppTitle) - 1, title, NULL); }

        const char* getGameTitle(ctxt_t* c) { return c->m_szAppTitle; }
...
#    endif
        ...
        void init(alloc_t* a, ctxt_t*& c)
        {
            // Allocator context
            void* ctxt_mem = a->allocate(sizeof(ctxt_t), sizeof(void*));
            c              = new (ctxt_mem) ctxt_t(a);
            sCtxt          = c;
...
#    endif
        ...
        void update(ctxt_t* c) {}

        //---------------------------------------------------------------------------------------------------------------------

        void shutdown(ctxt_t* c)
        {
            if (!IsDebuggerPresent())
            {
        ...
        bool        isCircleButtonBack() { return false; }
        s32         getNumCores(ctxt_t* c) { return 1; }
        s32         getNumHwThreadsPerCore(ctxt_t* c) { return 1; }
        u64         getCoreClockFrequency(ctxt_t* c) { return X_CONSTANT_U64(2 * 1024 * 1024 * 1024); }
        const char* getPlatformName(ctxt_t* c) { return "Win32"; }

        const char* getBuildConfigName(ctxt_t* c)
        {
...
#    endif
        ...
        const char* getBuildModeName(ctxt_t* c)
        {
#    if defined(TARGET_DEBUG)
            return "Debug";
...
#    endif
        ...
        EConsoleType getConsoleType(ctxt_t* c) { return c->m_console_type; }

        //---------------------------------------------------------------------------------------------------------------------

        EMediaType getMediaType(ctxt_t* c) { return c->m_media_type; }

        //---------------------------------------------------------------------------------------------------------------------

        ELanguage getLanguage(ctxt_t* c)
        {
        ...
        void setLanguage(ctxt_t* c, ELanguage language) { c->m_uLanguage = language; }

        const char* getLanguageStr(ctxt_t* c) { return gToString(c->m_uLanguage); }

        //---------------------------------------------------------------------------------------------------------------------

        u64 getTotalMemorySize(ctxt_t* c) { return c->m_uMemHeapTotalSize; }

        //---------------------------------------------------------------------------------------------------------------------

        u64 getCurrentSystemMemory(ctxt_t* c) { return c->m_uMemHeapTotalSize; }

        //---------------------------------------------------------------------------------------------------------------------

        void sleep(u64 uMicroSec) { SleepEx((u32)(uMicroSec / (u64)1000), FALSE); }

        //---------------------------------------------------------------------------------------------------------------------

        void createStackTrace(ctxt_t* c, u32 uStartIndex, u64* pTrace, u32& ruDepth, u32 uMaxDepth)
        {
